...
from app.services.campaign_dispatch import cancel_scheduled, send_campaign_background
...
from app.sms.phone import normalize, scrub_provider_text
import logging
...
async def cancel_campaign(request: Request, campaign_id: int,
                          db: Session = Depends(get_db), user: str = Depends(require_auth)):
    """Call off a scheduled send; the campaign stays as a draft.

    What cancelling means, and why it is refused on anything that has started,
    is decided in `campaign_dispatch.cancel_scheduled()` and not here. A refusal
    is a 409 carrying that function's sentence verbatim, so the rail shows the
    same words a post-mortem reads over the API.

    No rate limit: the limiter exists to cap what a stolen session can *spend*,
    and this spends nothing. `async def` like its neighbours; nothing about the
    race with the scheduler depends on it — the cancel's clear and the send
    path's flip to `running` are both conditional at the database
    (`campaign_dispatch.cancel_scheduled()`, `campaign_claim.take()`), so a
    `def` route in a threadpool worker would lose or win the same way.
    """
    if db.get(Campaign, campaign_id) is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    try:
        campaign = cancel_scheduled(db, campaign_id)
    except CampaignError as e:
        logger.warning(f"Campaign #{campaign_id} cancel REFUSED: {e}")
        raise HTTPException(status_code=409, detail=str(e))

    logger.info(f"Campaign #{campaign_id} cancelled by {get_client_ip(request)}")
    return {
        "success": True,
        "message": (f"Cancelled — \u201c{campaign.name}\u201d is back to a draft. Send it "
                    f"by hand from the rail, or leave it."),
