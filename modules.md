...
| L1 | **LiveAuctioneers bidder source** | Specced · next | 5n | `app/sources/{liveauctioneers,auction_scraper_base,platforms}.py`, `app/models/bidder_profile.py`, `app/services/{contact_service,scrape_runner}.py`, `app/core/config.py`, `app/main.py`, `requirements.txt`, `.env.example`, `alembic/versions/`, `tests/`, `agent/{accept-L1.sh,mutate-L1.py}` |
